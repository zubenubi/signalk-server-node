...
function findSelfDeltaValue (app, key) {
  ...
  let keyUpdate = rootUpdates.find(update => {
    return !_.isUndefined(update[key])
  })
  return _.isUndefined(keyUpdate) ? undefined : keyUpdate[key]
}

function setSelfSettings (app) {
  var name = findSelfDeltaValue(app, 'name')
  var mmsi = findSelfDeltaValue(app, 'mmsi')
  var uuid = findSelfDeltaValue(app, 'uuid')
