...
module.exports = function (app) {
  ...
  const snapshotPath = pathPrefix + versionPrefix + '/snapshot/'

  return {
    start: function () {
      app.use('/', express.static(__dirname + '/../../public'))

      app.get(apiPathPrefix + '*', handlerForPath(apiPathPrefix, true))
      app.get(snapshotPath + '*', handlerForPath(snapshotPath, false))

      function handlerForPath (_pathPrefix, serveRealTimeData) {
        return function (req, res, next) {
          var path = String(req.path).replace(_pathPrefix, '')
          ...
          if (req.query.time) {
            if (!app.historyProvider) {
              ...
              app.historyProvider.getHistory(
                ...
                deltas => {
                  if (deltas.length === 0) {
                    res.status(400).send('No data found for the given time')
                    return
                  }
          ...
          } else if (serveRealTimeData) {
            var last = app.deltaCache.buildFull(req.skUser, path)
            sendResult(last, path)
          } else {
            res.status(400)
            res.json({
              message: 'Please provide a timestamp for history snapshot data'
            })
          }
        }
      }
