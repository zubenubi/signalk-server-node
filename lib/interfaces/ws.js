...
const isIso8601 = /^(-?(?:[1-9][0-9]*)?[0-9]{4})-(1[0-2]|0[1-9])-(3[01]|0[1-9]|[12][0-9])T(2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9])(\\.[0-9]+)?(Z)?$/

module.exports = function (app) {
  'use strict'
  var _ = require('lodash')

  var debug = require('debug')('signalk-server:interfaces:ws')
  ...
  api.start = function () {
    ...
    primuses.push(new Primus(app.server, primusOptions))

    if (!_.isUndefined(app.historyProvider)) {
      const playbackOptions = JSON.parse(JSON.stringify(primusOptions))
      playbackOptions.pathname = '/signalk/v1/playback'
      const playbackPrimus = new Primus(app.server, playbackOptions)
      playbackPrimus.isPlayback = true
      primuses.push(playbackPrimus)
    }
    ...
    primuses.forEach(primus => {
      ...
      if (primus.isPlayback) {
        primus.use('playbackParamCheck', () => {
          return function (req, res, next) {
            if (!req.query.startTime || !isIso8601.test(req.query.startTime)) {
              res.statusCode = 400
              res.statusMessage =
                'Playback streaming requests must have a valid UTC ISO8601 startTime query parameter'
              res.end()
            } else {
              app.historyProvider.hasAnyData(
                {
                ...
                hasResults => {
                  if (hasResults) {
                    ...
                    res.statusCode = 400
                    res.statusMessage = 'No data for the given timeperiod'
                    res.end()
                  }
                }
              )
      ...
      primus.on('connection', function (spark) {
        debug(
          spark.id +
            ' ' +
            (primus.isPlayback ? '(playback)' : '') +
            ' connected with params ' +
            JSON.stringify(spark.query)
        )
        ...
        spark.on('data', function (msg) {
          ...
          if (primus.isPlayback) {
            const msg =
              'Playback streaming does not support upstream data like subscriptions'
            console.error(msg)
            spark.end({ msg })
            return
          }
          ...
          if (msg.subscribe) {
            if (spark.isHistory) {
              spark.end(
                'Subscribe messages not supported with history playback'
              )
              return
            }
        ...
        if (primus.isPlayback) {
          delete hello.timestamp
          hello.startTime = spark.query.startTime
          hello.playbackRate = spark.query.playbackRate || 1
        }
        spark.write(hello)
